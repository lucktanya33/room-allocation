...
export const CustomInputNumber: React.FC<CustomInputNumberProps> = ({
  ...
  return (
    <div className="flex items-center" onBlur={onBlur}>
      <button
        type="button"
        className={`border-cyan-400 text-cyan-400	w-12 h-12 border border-gray-300 rounded flex items-center justify-center ${
          disabledSetting.decrease ? "opacity-40 cursor-not-allowed" : ""
        }`}
      ...
      <input
        ...
        className={`mx-2 w-12 h-12 text-center border border-gray-300 rounded ${
          disabled ? "opacity-40 cursor-not-allowed" : ""
        }`}
      />
      <button
        type="button"
        className={`border-cyan-600 text-cyan-600	w-12 h-12 border border-gray-300 rounded flex items-center justify-center ${
          disabledSetting.increase ? "opacity-40 cursor-not-allowed" : ""
        }`}
        onClick={() => handleButtonClick(step)}
        onMouseDown={() => handleMouseDown(step)}
        onMouseUp={handleMouseUp}
